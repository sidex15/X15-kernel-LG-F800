...
#ifdef CONFIG_LGE_USB_MOISTURE_DETECTION_NO_UX
			if (chip->input_blocked && !chip->moisture_ux){
				...
				if((chip->usb_supply_type == POWER_SUPPLY_TYPE_USB) ||
					...
					chip->water_detect_chg_current = 500;
				else
					chip->water_detect_chg_current = 1000;
			} else {
				chip->water_detect_chg_current = 0;
				chip->hvdcp_not_supported = false;
			}

			rc = vote(chip->fcc_votable, WATERPROOF_FCC_VOTER, chip->input_blocked, chip->water_detect_chg_current);
			pr_smb(PR_LGE, "waterproof : FCC is %s.\n", chip->input_blocked ? "limited" : "released");
#else
...
#ifdef CONFIG_MACH_MSM8996_FALCON_LGU_LDU
	//LDU model does not support HVDCP.
	chip->hvdcp_not_supported = true;
	pr_err("LDU model does not support HVDCP. \n");
#endif
