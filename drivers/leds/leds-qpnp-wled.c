...
#if defined(CONFIG_LGE_DISPLAY_FALCON_COMMON)
static int qpnp_wled_set_cabc(struct qpnp_wled *wled, int level)
{
	int rc, i;
	u8 reg = 0;

	if(level > CABC_OFF_LEVEL){
		if(wled->en_cabc)
			return 0;
		else
			wled->en_cabc = 1;
	...
	for (i = 0; i < wled->num_strings; i++) {
		...
		rc = qpnp_wled_read_reg(wled, &reg,
				QPNP_WLED_CABC_REG(wled->sink_base, wled->strings[i]));
...
#if defined(CONFIG_LGE_DISPLAY_FALCON_COMMON)
	rc = qpnp_wled_set_cabc(wled, level);
	if (rc) {
		dev_err(&wled->spmi->dev, "wled set cabc failed\n");
	}
#endif
